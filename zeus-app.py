...
print(
    ...
    "some logs and metrics and finally, we will retrieve them.")
...
print("We are going to send a log " +`NUMBER_OF_SAMPLES`+ " times in groups of " +`BATCH_SIZE`+ ".")
...
for i in range(0, NUMBER_OF_SAMPLES, BATCH_SIZE):
    messages = [
        {"message": message + " - " + str(i + x)} for x in range(0, min(NUMBER_OF_SAMPLES - i, BATCH_SIZE))]
    print "Sending group " + str(i + 1) + ': ' + str(z.sendLog("Zeus101", messages))
...
for i in range(0, NUMBER_OF_SAMPLES, BATCH_SIZE):
    metrics = [
        {"value": random.randint(0, 100)} for x in range(0, min(NUMBER_OF_SAMPLES - i, BATCH_SIZE))]
    print "Sending group " + str(i + 1) + ': ' + str(z.sendMetric("Zeus101", metrics))
...
print("User token: " + token)
print(
    "\nCongratulations! You now have some logs and metrics in Zeus. Easy, right?")
...
# Log consumption
print(
    "\nGET request to http://api.ciscozeus.io/logs/" + token + "?log_name=Zeus101&limit=10&pattern=" + message)
...
if status == 200:
    ...
    print "\nThere are currently " + str(l['total']) + " logs in your Zeus account that match this query."
else:
    print "\nThere has been an error retrieving logs. Are the parameters correctly formatted?"

raw_input(
    "\nLets now retrieve some metrics from Zeus. Press ENTER when you are ready.")

# Metric consumption
print(
    "\nGET request to http://api.ciscozeus.io/metrics/" + token + "/_values?" +
    "limit=10&group_interval=1m&aggregator_function=sum&filter_condition='value > 90'")
...
if status == 200:
    ...
    print "There has been an error retrieving metrics. Are the parameters correctly formatted?"

raw_input(
    "\nFinally, let's retrieve a list of metrics. Press ENTER when you are ready.")
...
for metric in m:
    print metric
raw_input(
    "\nThis is the end of Zeus101. Please, feel free to use the ZeusClient class as a base for your code. Press ENTER to exit.")
